...
#define ONE_WIRE_BUS D6 // Пин подключения OneWire шины, 0 (D3)
...
void loop()
{
  ...
  sensors.requestTemperatures();
   Serial.print(sensors.getTempCByIndex(0));   // выводим температуру на монитор
  Serial.println("C");
 ...
 /* for (int i = 0; i < deviceCount; i++)
  {
    ...
    sensor = sensors.getTempC(temperatureSensors[i]);
  }*/
 // delay(1000);
if (irrecv.decode(&results)) {
    ...
    }else if(results.value == 0xFFE01F) // Minus
    {
       ...
       else if(reseme == 2)
       {
        ...
        SAVE_HUMID = SAVE_HUMID - 1; 
        if(SAVE_HUMID < 10.0) SAVE_HUMID = 99.0;
       }
       else if(reseme == 3)
       {
    ...
    else if(results.value == 0xFFA857) // Plus
    {
       ...
       else if(reseme == 2)
       {
        ...
        SAVE_HUMID = SAVE_HUMID + 1;
        if(SAVE_HUMID > 99.0) SAVE_HUMID = 10.0;
       }
       else if(reseme == 3)
       {
        SAVE_TIMER = SAVE_TIMER + 0.1;
 ...
 if(state == 0)
 {
    ...
    lcd.setCursor(0,0);
    lcd.printf("\2 %03.1f\3 \4 %d \% \6",sensor,DHT.temperature); // DHT.humidity
    lcd.setCursor(0,1);
    lcd.printf("\5 %02.1f\3 \4 %01.1f\% \7",SAVE_TEMP, SAVE_HUMID);
    lcd.setCursor(0,1);
    lcd.printf("\5 %02.1f\3 \4 %01.1f\% |",SAVE_TEMP, SAVE_HUMID);
    lcd.setCursor(0,1);
    lcd.printf("\5 %02.1f\3 \4 %01.1f\% \7",SAVE_TEMP, SAVE_HUMID);
    lcd.setCursor(0,1);
    lcd.printf("\5 %02.1f\3 \4 %01.1f\% |",SAVE_TEMP, SAVE_HUMID);
